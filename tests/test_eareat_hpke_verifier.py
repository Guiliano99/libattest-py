...
from libattest.formats.csrattest import prepare_attestation_bundle, prepare_attestation_statement
...
def _bundle_der(
    ...
    *,
    ...
    inner = jose_jws.sign_es256(
        {"eat_nonce": jose_jws.b64u_encode(nonce), "mock_claim": mock_claim, "iat": 0},
        inner_signer or attester,
    )
    jwe = jose_hpke.seal_integrated(
        inner.encode(),
        {"kid": "eareat-hpke-verifier", "eat_nonce": jose_jws.b64u_encode(nonce)},
    ...
    if tamper:
        ...
        jwe = ".".join(parts)
    cmw_der = der_encoder.encode(char.UTF8String(json.dumps(["application/jose", jwe, 4])))
...
def test_cmw_stmt_is_utf8string() -> None:
    attester = ec.generate_private_key(ec.SECP256R1())
    verifier = _make_verifier(attester)
    from libattest.formats.csrattest import decode_attestation_bundle
