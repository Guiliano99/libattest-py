...
def test_nonce_request_uses_req_type_info() -> None:
    ...
    assert str(request["reqTypeInfo"]["type"]) == eareat_hpke.EVIDENCE_ENC_PARAMS_OID
...
def test_nonce_response_uses_resp_type_info() -> None:
    ...
    type_info["type"] = eareat_hpke.EVIDENCE_ENC_PARAMS_OID
    ...
    assert str(decoded["respTypeInfo"]["type"]) == eareat_hpke.EVIDENCE_ENC_PARAMS_OID
